            ...
            if (fields.length == CSV_HEADER_FIELDS) {
                ...
                for (int i = 1; i < csv.size(); ++i) {
                    ...
                    if (fields.length == CSV_DATA_FIELDS) {
                        ...
                        String puzzleidColumn = properties.getProperty("sql_field_puzzleid");
                        String rowColumn = properties.getProperty("sql_field_row");
                        String colColumn = properties.getProperty("sql_field_column");
                        String boxColumn = properties.getProperty("sql_field_box");
                        String wordColumn = properties.getProperty("sql_field_word");
                        String clueColumn = properties.getProperty("sql_field_clue");
                        String directionColumn = properties.getProperty("sql_field_direction");

                        params = new HashMap<>();

                        params.put(puzzleidColumn, String.valueOf(puzzleid));
                        params.put(rowColumn, fields[0]);
                        params.put(colColumn, fields[1]);
                        params.put(boxColumn, fields[2]);
                        params.put(directionColumn, fields[3]);
